...
>
> **Target:** `target-webview-app.apk` if available in `targets/`. If not present, use `target-kyc-basic.apk` as a fallback -- any app with `WebView.loadUrl()` calls will work. See `targets/README.md` for build instructions.
