...
>
> **Target:** `target-unknown.apk` if available in `targets/`. If not present, use `target-kyc-basic.apk` as the "mystery" target -- pretend you do not know what it contains and let the automation discover it.
