---
title: Android Biometric Bypass
description: APK tampering, camera injection, GPS spoofing & sensor manipulation against Android identity verification
template: splash
hero:
  title: Android Biometric Bypass
  tagline: "APK tampering, camera injection, GPS spoofing & sensor manipulation — defeating Android identity verification from first principles to full engagements."
  actions:
    - text: Get started
      link: /AndroidRedTeam/book/foundations/01-threat-landscape/
      icon: right-arrow
    - text: Jump to Labs
      link: /AndroidRedTeam/labs/00-environment-verification/
      icon: right-arrow
      variant: minimal
---

import { Card, CardGrid, Steps, Aside } from '@astrojs/starlight/components';

<Aside type="caution" title="Authorized Use Only">
For security professionals conducting **authorized** assessments only.
</Aside>

<CardGrid stagger>
	<Card title="The Book" icon="open-book">
		18 chapters — threat landscape, injection techniques, full engagements, advanced evasion, and defense.
	</Card>
	<Card title="The Labs" icon="laptop">
		14 hands-on exercises with real APKs, self-check scripts, and concrete deliverables.
	</Card>
	<Card title="Materials Kit" icon="seti:folder">
		Target APKs, payload configs (GPS, sensors, frames), and automation scripts.
	</Card>
	<Card title="Quick Reference" icon="list-format">
		Every command, payload format, and troubleshooting tip on one page.
	</Card>
</CardGrid>

---

## The Engagement Cycle

<Steps>

1. **Recon** — Decode the APK, map every hookable surface

2. **Patch** — Instrument the bytecode with the patch-tool

3. **Configure** — Push payloads (frames, GPS configs, sensor profiles)

4. **Execute** — Run the target flow with all injection subsystems active

5. **Report** — Capture evidence, compute statistics, write findings

</Steps>

You'll learn each phase independently, then combine them in full engagements against multi-step verification flows.

---

## AI-Assisted Workflow

Use [Cursor](https://cursor.sh/)? This repo includes two **Agent Skills** that give the AI the full red team methodology — recon patterns, smali techniques, injection configs, and a post-patch verification checklist. Copy them to `.cursor/skills/` and the agent can decode, patch, verify, and diagnose like a practitioner. See [Chapter 4](/AndroidRedTeam/book/foundations/04-the-lab/) for setup.

---

## Who Is This For

Penetration testers, security engineers, and developers working on **authorized** assessments of Android identity verification systems.

**No prior reverse engineering experience required** — Part I covers the foundations.
